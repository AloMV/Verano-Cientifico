...
const props = defineProps({
    ...
    proyectStatuses: { type: Object, required: true },
...
const form = useForm({
    _method: 'patch',
    id: props.proyect.id,
    title: props.proyect.title,
    type: props.proyect.type,
    abstract: props.proyect.abstract,
    key_works: props.proyect.key_works,
    knowledge_area_id: props.proyect?.knowledge_area?.id,
    proyect_status_id: props.proyect.proyect_status_id,
    call: props.proyect?.call?.title,
    file: null,

    authors: props.proyect.authors,
    selectedArea: props.proyect?.knowledge_area?.parent_id,
});

const saveForm = () => {
    form.post(route(`${props.routeName}update`, props.proyect.id));
};
            ...
            <CardBox isForm @submit.prevent="saveForm" class="col-span-3">
                ...
                <BaseButtons class="mt-5">
                    <BaseButton :routeName="`${props.routeName}index`" :icon="mdiClose" color="white"
                        label="Cancelar" />
                    <BaseButton @click="saveForm" :icon="mdiContentSave" type="submit" color="success" label="Actualizar proyecto" />
                </BaseButtons>
            </CardBox>
