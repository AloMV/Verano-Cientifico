...
const saveEditor = () => {
    formEditor.patch(route(`${props.routeName}updateEvaluation`, props.article.id));
}
...
<template>
    ...
    <div v-if="getRole(['Revisor'])">
        ...
        <div class="lg:mb-5">
            <FormField required label="Completa todos los criterios:" :error="formReviewer.errors.criteria">
    ...
    <div v-else-if="getRole(['Editor', 'Admin'])">
        ...
        <p class="text-slate-500 text-sm mb-2">
            Los comentarios y resultado final se verán reflejados en el estatus del alumno.
        </p>
        ...
        <BaseButtons class="mt-5">
            <BaseButton @click="saveEditor" :icon="mdiContentSave" color="success" label="Guardar" small />
        </BaseButtons>
                    ...
                    <DropdownItem :value="false" class="">
                        <template #header>
                            <div class="flex items-center justify-between w-full space-x-2">
                                ...
                                <span :class="item.article_status?.class">
                                    {{ item.article_status?.name }}
                                </span>
                            </div>
                        </template>

                        <p class="text-center text-xs text-slate-700">Criterios</p>
                        ...
                        <BaseDivider />
                        <p class="text-sm font-medium text-gray-900 dark:text-gray-300">Comentarios:</p>
                        <p class="text-sm text-gray-700 dark:text-gray-400">{{ item.comment }}</p>
                    </DropdownItem>
                </li>
            </ul>
        </div>
