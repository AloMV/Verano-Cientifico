...
class ArticleController extends Controller
{
    ...
    public function show(Article $article)
    {
        ...
        return Inertia::render("{$this->source}Show", [
            ...
            'article'           => $article,
            'filePath'          => $this->getFile($article->id),
    ...
    private function getFile($id)
    {
        ...
        if ($file && Storage::disk('private')->exists($file->path)) {
            return URL::signedRoute('file.serve', $file->id, now()->addMinutes(60));
        }
        return null;
    }
    ...
    private function sendEmail($id)
    {
        ...
        foreach ($allAdmins as $admin) {
            Mail::to($admin)->send(new StoreArticle($admin->name, $userData));
        }
    }

    public function updateEvaluation(EvaluationEditorRequest $request, $id)
    {
        $article = Article::findOrFail($id);
        $this->authorize('updateEvaluation', $article);
        ...
        Mail::to($article->postulant->email)->send(new EvaluationArticle($postulantData));
    ...
    private function canArticlesAll()
    {
        $user = User::find(Auth::id());
